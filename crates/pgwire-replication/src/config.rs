...
pub struct ReplicationConfig {
    ...
    /// Enable pgoutput protocol v2 streaming for large transactions (PG14+).
    ///
    /// When enabled, PostgreSQL streams large in-progress transactions incrementally
    /// instead of buffering them until commit. This reduces server-side memory pressure
    /// and WAL retention for large transactions.
    ///
    /// Requires PostgreSQL 14 or later. If the server doesn't support protocol v2,
    /// the connection will fail.
    ///
    /// Default: false (uses protocol v1)
    pub streaming: bool,
}
