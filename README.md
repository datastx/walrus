...
For those previously-streaming tables, Walrus marks them with a resync flag. This flag tells the Iceberg Writer that the table's existing data may not match the source and needs reconciliation. When the writer encounters backfill files for a table flagged for resync, it does not simply append the new data or drop the existing table. Instead, it diffs the new snapshot against the existing Iceberg data by primary key and writes only the actual changes — rows that were added or modified become upserts, and rows that no longer exist in the source become deletes. Rows that are identical in both the old and new data are skipped entirely.

This merge approach has two significant advantages over dropping and rebuilding the table. First, it preserves the Iceberg table's snapshot history, so downstream consumers that depend on time travel queries continue to work. Second, it is far more efficient for large tables where only a small number of rows changed during the outage window. A billion-row table with three changed rows produces three upserts and zero deletes, rather than rewriting every row. The diff is computed in memory using a set comparison, so the cost is proportional to the amount of data that actually changed rather than the total table size.

After the resync merge completes and the writer finishes processing all the backfill files, the table transitions back to streaming and the resync flag is cleared. From that point forward, the table behaves exactly as it would after a normal first-time backfill. Any stale CDC files from before the re-bootstrap are discarded during the reset, since their positions refer to the old slot and have no meaning in the context of the new one.

Operators should be aware that a resync can happen whenever the service is down long enough for the replication slot to be invalidated. When it occurs, the writer logs the event with the number of upserts and deletes that resulted from the merge, so ops teams can see exactly what changed and verify that the reconciliation behaved as expected.
